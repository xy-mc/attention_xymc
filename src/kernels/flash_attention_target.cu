...
// Raw pointer launcher (no Torch deps) to call the kernel for given D and stages
template <const int kHeadDim, const int kStage>
static void launch_flash_attn_mma_stages_split_q_raw(half *Q, half *K, half *V,
                                                     half *O, int QKV_batch,
                                                     int QKV_head,
                                                     int QKV_seqlen,
                                                     cudaStream_t stream) {
  ...
  flash_attn_mma_stages_split_q_kernel<
      ...
      kMmaTileSeqLenK, kMmaTileSeqLenP, kMmaTileHeadDimV, kWarpTileSeqLenQ,
      kWarpTileSeqLenK, kWarpTileSeqLenP, kWarpTileHeadDimV, kStage, kPad>
      <<<grid, block, smem_max_size, stream>>>(Q, K, V, O, QKV_seqlen,
                                               QKV_head);
}

// Exposed C ABI for cross-TU invocation
extern "C" void flash_attn_target_launch_half(const half *Q, const half *K,
  ...
  if (stages > 1) {
    switch (D) {
    case 32:
      launch_flash_attn_mma_stages_split_q_raw<32, 2>(const_cast<half *>(Q),
                                                      const_cast<half *>(K),
                                                      const_cast<half *>(V), O,
                                                      B, H, N, stream);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_raw<64, 2>(const_cast<half *>(Q),
                                                      const_cast<half *>(K),
                                                      const_cast<half *>(V), O,
                                                      B, H, N, stream);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_raw<96, 2>(const_cast<half *>(Q),
                                                      const_cast<half *>(K),
                                                      const_cast<half *>(V), O,
                                                      B, H, N, stream);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_raw<128, 2>(const_cast<half *>(Q),
                                                       const_cast<half *>(K),
                                                       const_cast<half *>(V),
                                                       O, B, H, N, stream);
      break;
    ...
    switch (D) {
    case 32:
      launch_flash_attn_mma_stages_split_q_raw<32, 1>(const_cast<half *>(Q),
                                                      const_cast<half *>(K),
                                                      const_cast<half *>(V), O,
                                                      B, H, N, stream);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_raw<64, 1>(const_cast<half *>(Q),
                                                      const_cast<half *>(K),
                                                      const_cast<half *>(V), O,
                                                      B, H, N, stream);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_raw<96, 1>(const_cast<half *>(Q),
                                                      const_cast<half *>(K),
                                                      const_cast<half *>(V), O,
                                                      B, H, N, stream);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_raw<128, 1>(const_cast<half *>(Q),
                                                       const_cast<half *>(K),
                                                       const_cast<half *>(V),
                                                       O, B, H, N, stream);
      break;
...
#ifdef ENABLE_TORCH_LAUNCH
// Original Torch-based wrapper (disabled by default)
template <const int kHeadDim, const int kStage>

...
void launch_flash_attention_v1_forward(
    ...
    cudaFuncSetCacheConfig(flash_attention_v1_forward_kernel, cudaFuncCachePreferShared);
    // Set attribute to requested size if within opt-in limit
    if ((int)smem_size <= max_optin) {
        cudaFuncSetAttribute(
            flash_attention_v1_forward_kernel,
    ...
    flash_attention_v1_forward_kernel<<<grid, block, smem_size, stream>>>(Q, K, V, O, B, H, N, D, scale, Br, Bc, Tr, Tc,
    d_l, d_m);
